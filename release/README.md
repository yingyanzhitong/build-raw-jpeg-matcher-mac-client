...
4. 把 `version`、`pub_date`、`notes`、`installers` 中的安装包 `url`、`platforms` 中的 updater `url` 和 `signature` 替换为真实值。
5. 确认 `installers.*.url` 指向可手动安装的安装包，`platforms.*.url` 指向 updater 附件后，将 `release/latest.json` 推送到 Gitee 仓库 main 分支。
