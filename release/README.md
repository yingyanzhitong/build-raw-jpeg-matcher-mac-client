...
6. 全部 Gitee 附件上传成功后，写入 Gitee 仓库 main 分支的 `release/latest.json`，清单里的安装包下载地址指向 Gitee Release。
...
5. 确认 `url` 指向 Gitee Release 附件后，将 `release/latest.json` 推送到 Gitee 仓库 main 分支。
