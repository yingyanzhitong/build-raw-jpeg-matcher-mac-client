...
5. 写入 Gitee 仓库 main 分支的 `release/latest.json`，清单里的安装包下载地址指向 GitHub Release。
...
1. 在 GitHub Release 上传对应平台的 updater artifact 和 `.sig` 文件。
2. 复制 `release/latest.example.json` 为 `release/latest.json`。
3. 把 `version`、`pub_date`、`notes`、各平台 `url` 和 `signature` 替换为真实值。
4. 将 `release/latest.json` 推送到 Gitee 仓库 main 分支。
