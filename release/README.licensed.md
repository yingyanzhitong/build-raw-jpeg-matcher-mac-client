...
- 主更新清单：`https://gitee.com/masongzhi1/raw-jpeg-matcher-licensed-release/raw/main/release/latest.json`
- 备用更新清单：`https://github.com/yingyanzhitong/build-raw-jpeg-matcher-mac-client/releases/latest/download/latest.json`
...
6. 工作流将 GitHub 备用更新清单作为 Release 的 `latest.json` 资源发布，再把更新包和 `release/latest.json` 同步到独立 Gitee 仓库；无需向受保护的 `licensed` 分支写入，Gitee 的 DMG 上传也不会阻塞自动更新清单。
