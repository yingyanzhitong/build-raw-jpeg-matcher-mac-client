...
## 许可证服务 1.2.0 - 2026-07-27

- 将鉴权运行时从 EdgeOne Cloud Functions 迁移到腾讯云 SCF 广州地域的 Node.js 18 Web 函数，生产客户端固定使用腾讯云公开函数 URL，不再依赖自定义域名。
- SCF 继续读取现有 EdgeOne Blob，许可证摘要、设备绑定、审计事件、管理员、会话和限流数据无需迁移。
- 新增 SCF HTTP 适配器、Web Crypto 兼容处理、部署包构建和线上健康检查；根路径会跳转到账号密码管理后台。
- 激活版升级到 `1.0.1`，通过独立 `licensed-v1.0.1` tag 构建三平台安装包并同步 Gitee 更新清单；同步脚本不上传额外源码附件，无激活版版本和行为保持不变。
